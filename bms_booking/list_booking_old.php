                ...
                <table id="example2" class="table table-bordered table-hover table-striped" role="grid" width="100%">
                  ...
                  <tbody>
                    <?php 
					                      ...
					                      $Query = "SELECT tb1.*,tb2.`location_name` AS `pickup_location`,tb3.`location_name` AS `dropoff_location`,tb4.`vehicle_name` AS `vehicle_name`,tb5.`broker_name` AS `broker_name` FROM `tbl_bms_booking` tb1 LEFT JOIN `tbl_bms_location_master` tb2 ON tb1.`pickUpLocation`=tb2.`location_id`  LEFT JOIN `tbl_bms_location_master` tb3 ON tb1.`returnLocation`=tb3.`location_id` LEFT JOIN `tbl_bms_vehicle_master` tb4 ON tb1.`vehicle_id`=tb4.`vehicle_id` LEFT JOIN `tbl_bms_broker_master` tb5 ON tb1.`broker_id`=tb5.`broker_id` ORDER BY tb1.`entered_date` DESC";
                    ...
                    <tr>
                      ...
                      <?php if(empty($build['fleet_no']) && $build['status']==1){?><a href="edit_booking.php?flag=<?=$build['booking_id']?>&gtoken=<?=$token?>" title="edit" class="btn btn-block btn-default btn-sm"/><i class="fa fa-edit fa-sm"></i></a><?php }?>
                      <a href="view_booking.php?flag=<?=$build['booking_id']?>&gtoken=<?=$token?>" title="View" class="btn btn-block btn-default btn-sm"/><i class="fa fa-eye fa-sm"></i></a>
                      <a href="booking_pdf.php?flag=<?=$build['booking_id']?>&gtoken=<?=$token?>" title="pdf" target="_blank" class="btn btn-block btn-default btn-sm"/><i class="fa fa-file-pdf-o fa-sm" aria-hidden="true">                      </i>
                      </a>
                      </td>
                      <td data-order="<?=date('Y-m-d',strtotime($build['entered_date']))?>"><?=date('d-m-Y H:i:s',strtotime($build['entered_date']))?></td>
                        ...
                        <!-- <td><?=$build['customerEmail']?></td>-->
                         <td><?=$build['customerPhone']?></td>
