...
if ($refreshflag==3){
	...
	$np_name            = killstring($_REQUEST['np_name']);
	$np_description     = killstring($_REQUEST['np_description']);
	$IsPageValid = true;	
	if(empty($np_name)){
	$msg   = "Please enter netwrok provider name!";
	$IsPageValid = false;
	...
	if($IsPageValid==true){//valid - true
		  $sql         = "INSERT INTO `tbl_cmt_network_providers`(`company_id`, `np_name`, `np_description`, `entered_by`) VALUES ('$company','$np_name','$np_description','$userno')";
		  $result      = mysqli_query($conn,$sql); 
		  if($result){
		  $msg = "Network provider added successfully";   
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>New Network Provider</h1>
    <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active">New Network Provider</li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <form role="form" name="company-form" id="company-form" method="post" action="#" enctype="multipart/form-data">
      ...
      <div class="box box-default">
        <div class="box-header with-border">
          <h3 class="box-title">Network Provider</h3>
        ...
        <div class="box-body">
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="form-group">
                <label class="required">Network Provider Description:</label>
                <textarea class="validate[required] form-control" name="np_description" placeholder="Network Provider Description"/></textarea>
              </div>
            </div>
            <div class="col-md-6">
             <div class="form-group">
                <label class="required">Network Provider Name:</label>
                <input type="text" class="validate[required] form-control" name="np_name" placeholder="Network Provider Name"/>
              </div>
            </div>
          </div>
          <!-- /.row -->
            ...
            <div class="col-md-12">
              <div class="table-responsive">
                <table id="example2" class="table table-bordered table-hover table-striped" role="grid" width="100%;">
                  ...
                  <tbody>
                    <?php 
				                           $sqlQ  = mysqli_query($conn,"SELECT tb1.*,tb2.`company_name` FROM `tbl_cmt_network_providers` tb1 LEFT  JOIN `tbl_cmt_ins_company_master` tb2 ON tb1.`company_id`=tb2.`company_id` ORDER BY tb1.`entered_date` DESC");
										   $itr = 0;
