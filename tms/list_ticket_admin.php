...
$subqry  = "";
$subqryA = "";
if($_SESSION['is_IT_SA']==0){
$subqry = " WHERE tb1.`assigned_to`='$user_id' ";	
$subqryA = "  AND `assigned_to`='$user_id' ";	
}


$SqlNewQryA                = mysqli_query($conn,"SELECT  * FROM `tbl_tms_ticket` WHERE `priority`=3 AND `status`=1 ".$subqryA."");
$high_open                 =(mysqli_num_rows($SqlNewQryA))?mysqli_num_rows($SqlNewQryA):0; 	 
$SqlNewQryB                = mysqli_query($conn,"SELECT  * FROM `tbl_tms_ticket` WHERE `priority`=2 AND `status`=1 ".$subqryA."");
$medium_open               =(mysqli_num_rows($SqlNewQryB))?mysqli_num_rows($SqlNewQryB):0;
$SqlNewQryC                = mysqli_query($conn,"SELECT  * FROM `tbl_tms_ticket` WHERE `priority`=1 AND `status`=1 ".$subqryA."");
$low_open                  =(mysqli_num_rows($SqlNewQryC))?mysqli_num_rows($SqlNewQryC):0;
$SqlNewQryF                = mysqli_query($conn,"SELECT  * FROM `tbl_tms_ticket` WHERE `priority`=3 AND `status`=2 ".$subqryA."");
$high_pending              =(mysqli_num_rows($SqlNewQryF))?mysqli_num_rows($SqlNewQryF):0;
$SqlNewQryD                = mysqli_query($conn,"SELECT  * FROM `tbl_tms_ticket` WHERE `priority`=2 AND `status`=2 ".$subqryA."");
$medium_pending            =(mysqli_num_rows($SqlNewQryD))?mysqli_num_rows($SqlNewQryD):0;
$SqlNewQryE                = mysqli_query($conn,"SELECT  * FROM `tbl_tms_ticket` WHERE `priority`=1 AND `status`=2 ".$subqryA."");
$low_pending               =(mysqli_num_rows($SqlNewQryE))?mysqli_num_rows($SqlNewQryE):0;
?>
<style>
 table#example2 {
  margin: 0 auto;
  ...
  <section class="content">
    <form role="form" name="company-form" id="company-form" method="post" action="#" enctype="multipart/form-data">
      ...
      <div class="box box-default">
        ...
        <!-- /.box-header -->
      <div class="box box-default">
      <div class="box-body" style="text-align: center;width: 80%;margin: 0 auto;" >
            <table class="table table-bordered" style="text-align:center; width:75% !important;">
              <thead>
                <tr>
                  <th style="text-align:center; background:#000000; color:#FFFFFF;"></th>
                  <th style="text-align:center; background:#000000; color:#FFFFFF;">High</th>
                  <th style="text-align:center; background:#000000; color:#FFFFFF;">Medium</th>
                  <th style="text-align:center; background:#000000; color:#FFFFFF;">Low</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td style="background:#000000; color:#FFFFFF;">Open</td>
                  <td style="color:#FFFFFF; background:#F8696B;"><?php if($high_open==0){ echo $high_open;}else{?><a style="color: white;
    font-weight: bolder;" href="javascript:void(0)" id="funHighOpen"><?=$high_open?></a><?php }?></td>
                  <td style="color:#FFFFFF; background:#FDD17F;"><?php if($medium_open==0){ echo $medium_open;}else{?><a style="color: white;
    font-weight: bolder;"href="javascript:void(0)" id="funMediumOpen"><?=$medium_open?></a><?php }?></td>
                  <td style="color:#FFFFFF; background:#E0E383;"><?php if($low_open==0){ echo $low_open;}else{?><a style="color: white;
    font-weight: bolder;"href="javascript:void(0)" id="funLowOpen"><?=$low_open?></a><?php }?></td>
                </tr>
                <tr>
                  <td style="background:#000000; color:#FFFFFF;">In Progress</td>
                  <td style="color:#FFFFFF; background:#FA9D75;"><?php if($high_pending==0){ echo $high_pending;}else{?><a style="color: white;
    font-weight: bolder;"href="javascript:void(0)" id="funHighPending"><?=$high_pending?></a><?php }?></td>
                  <td style="color:#FFFFFF; background:#A2D07F;"><?php if($medium_pending==0){ echo $medium_pending;}else{?><a style="color: white;
    font-weight: bolder;"href="javascript:void(0)" id="funMediumPending"><?=$medium_pending?></a><?php }?></td>
                  <td style="color:#FFFFFF; background:#63BE7B;"><?php if($low_pending==0){ echo $low_pending;}else{?><a style="color: white;
    font-weight: bolder;"href="javascript:void(0)" id="funLowPending"><?=$low_pending?></a><?php }?></td>
                </tr>
              </tbody>
            </table>
          </div>
               ...
               <table id="example2" class="table table-bordered table-hover table-striped stripe row-border order-column" role="grid" style="font-size:13px;">
                  ...
                  <tbody>
                    <?php 
					                        ...
					                        $sql  = "SELECT tb1.`ticket_id`, tb1.`ticket_no`, tb1.`subject`, tb1.`description`, tb1.`priority`, tb1.`assigned_to`,tb1.`estimated_date`, tb1.`entered_by`,tb1.`entered_date`,tb1.`updated_date`,tb1.status,tb2.`status_name`,tb3.`full_name` AS `requestor`,tb4.`full_name` AS `assignee`,tb5.`priority_name` AS `priority_name`,tb6.`name` AS `module_name`,tb7.`sub_name` AS `sub_module_name`  FROM `tbl_tms_ticket` tb1 LEFT JOIN `tbl_tms_status_master` tb2 ON tb1.`status`=tb2.`status_id` LEFT JOIN `tbl_login` tb3 ON tb1.`entered_by`=tb3.`user_id` LEFT JOIN `tbl_login` tb4 ON tb1.`assigned_to`=tb4.`empid` LEFT JOIN `tbl_tms_priority_master` tb5 ON tb1.`priority`=tb5.`priority_id` LEFT JOIN `tbl_tms_module` tb6 ON tb1.`module_id`=tb6.`module_id`  LEFT JOIN `tbl_tms_sub_module` tb7 ON tb1.`sub_module_id`=tb7.`sub_module_id` ".$subqry." ORDER BY tb1.`estimated_date` ASC";
				...
				<?php /*?>	<td><a href="javascript:void(0)" data-toggle="tooltip" title="<?=$build['description']?>"><?=$task_description?></a></td><?php */?>
                   ...
                   <?php /*?> <td data-order="<?=!empty($build['estimated_date'])?date('d-m-Y',strtotime($build['estimated_date'])):NULL;?>"  style="display:none;"><?=!empty($build['estimated_date'])?date('d-m-Y',strtotime($build['estimated_date'])):NULL;?></td>
                    ...
                    <?php if($build['status']!=4 && $build['status']!=5){?>
                    <a href="edit_ticket_admin.php?flag=<?=$build['ticket_id']?>&gtoken=<?=$token?>" title="UPDATE" class="btn btn-default btn-xs"><i class="fa fa-edit fa-sm"></i></a>
                   <?php }?>   
                    </td>
                  </tr>
                  <!---------------Preview show------------>
...
$(document).ready(function() {
 var oTable = $('#example2').dataTable({
  ...
  "aoColumnDefs": [
        { "bSortable": false, "aTargets": [ 1, 3,8] }
    ],
		 "aaSorting": [[ 0, "desc" ]]
   });
	...
	$('#myStatus').on('change',function(){
        ...
        oTable.fnFilter("^"+selectedValue+"$", 6, true); //Exact value, column, reg
		}
    });
	...
	$('#funHighOpen').on('click',function(){
        ...
        oTable.fnFilter("^"+selectedValue+"$", 6, true); //Exact value, column, reg
		oTable.fnFilter("^"+selectedValueA+"$", 7, true); //Exact value, column, reg
    });
