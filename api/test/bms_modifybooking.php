...
if(!empty($uname) && !empty($pwd)){
	...
	if($NumNewQry>0){
		...
		/********************************Here goes the insertion codes********/
	     ...
	     $query = mysqli_query($conn,"UPDATE `tbl_bms_booking_test` SET `customerName`='$customerName',`customerPhone`='$customerPhone',`flightNumber`='$flightNumber',`updated_date`=NOW() WHERE `reservation_no`='$reservationNumber' AND `broker_id`='$broker_id'");
			 if($query){
