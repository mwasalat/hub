...
/*************Location Buffer Time Calculation*******/
		...
		/**********Conditions*************/ 
		if(($returnLocation==19 && $pickUpLocation!=19) || ($returnLocation==30 && $pickUpLocation!=30)){
		$Qry   = "";	
		}else{
				if($a_buffer_time_to_check >= $b_current_date_time){//buffer time check	
				$Qry      = "SELECT `vehicle_id`,`vehicle_name`, `group_name` ,IFNULL((SUM(`total`)  + `airport_fee` + `return_airport_fee` + `inter_emirate_pricing`) - ROUND((SUM(`total`)+ `airport_fee` + `return_airport_fee` + `inter_emirate_pricing`)*('".$broker_vat_percentage."'/100),2),0) AS `valueWithoutTax`,IFNULL(ROUND((SUM(`total`) + `airport_fee` + `return_airport_fee` + `inter_emirate_pricing`) * ('".$broker_vat_percentage."'/100),2), 0) AS `taxValue`,'".$broker_vat_percentage."' AS `taxRate`,
				IFNULL((SUM(`total`) + `airport_fee` + `return_airport_fee` + `inter_emirate_pricing`),0) AS `totalValue`,IFNULL(SUM(`cdw`),0) AS `cdw`,IFNULL(SUM(`scdw`),0) AS `scdw`,IFNULL(SUM(`pai`),0) AS `pai`,IFNULL(SUM(`gps`),0) AS `gps`,IFNULL(SUM(`baby_seat`),0) AS `baby_seat`
				FROM (SELECT tb2.`vehicle_id` AS `vehicle_id`, tb2.`vehicle_name` , tb3.`group_name`,
				...
				tb1.`rate` AS `rate`,tb1.`cdw` AS `cdw`,tb1.`scdw` AS `scdw`,tb1.`pai` AS `pai`,tb1.`gps` AS `gps`,tb1.`baby_seat` AS `baby_seat`
