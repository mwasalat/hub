...
if(!empty($uname) && !empty($pwd)){ 
	...
	if($NumNewQry>0){
		...
		/********************************Here goes the insertion codes********/
	     ...
	     $query = mysqli_query($conn,"UPDATE `tbl_bms_booking_test` SET `status`='2',`updated_date`=NOW() WHERE `reservation_no`='$reservationNumber' AND `broker_id`='$broker_id'");
			 if($query){
				 /********Inventory Matching*************/
				 $sqlA = mysqli_query($conn,"SELECT `pickUpDate`,`returnDate`,`pickUpLocation`,`vehicle_id` FROM `tbl_bms_booking_test` WHERE `reservation_no`='$reservationNumber' AND `broker_id`='$broker_id'");
...
while($build = mysqli_fetch_array($data)){ 
  ...
  $entered_date = date('d.M.Y H:i:s',strtotime($build['entered_date']));
				...
				//$cc             = "marketing@autostrad.com,reservations@autostrad.com,sanil.sadasivan@onetechnology.biz";
				$to             = "sanil.sadasivan@onetechnology.biz"; 
				//$cc             = "hashim.pudiyapura@onetechnology.biz,".$return_location_email;
