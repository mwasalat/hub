...
if(!empty($uname) && !empty($pwd)){
	...
	if($NumNewQry>0){
		$sql      = mysqli_query($conn,"SELECT tb1.`location_id`,tb1.`location_name`,tb2.`city_name` AS `location_city`,tb3.`nicename` AS `location_country`,tb1.`location_latitude` AS `location_latitude`,tb1.`location_longitude` AS `location_longitude`,tb1.`location_landline` AS `location_landline`,tb1.`location_email` AS `location_email`,tb1.`location_iata` AS `location_iata`, tb1.`is_active` AS `is_active`,tb1.`updated_date` AS `updated_date`,tb1.`entered_date` AS `entered_date` from `tbl_bms_location_master` tb1 LEFT JOIN `tbl_bms_city_master` tb2 ON tb1.`location_city`=tb2.`city_id` LEFT JOIN `tbl_country` tb3 ON tb1.`location_country`=tb3.`id` WHERE tb1.`is_active`=1 ORDER BY tb1.`location_name` ASC");
		$resL     = array();
