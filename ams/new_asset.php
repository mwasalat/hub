...
if ($refreshflag==3){
	...
	if($IsPageValid==true){//valid - true
	             ...
	             /******************Auto Transaction no***********************/
			        $QueryCounter  =  mysqli_query($conn,"SELECT `auto_id`+1 as `id` from `tbl_ams_asset_transaction_no`");
					$rowQryA       = mysqli_fetch_row($QueryCounter);
					$value2A       = $rowQryA[0];
					$QueryCounterA =  mysqli_query($conn,"UPDATE `tbl_ams_asset_transaction_no` set `auto_id`='$value2A'");
					$ordervalue    = "AST-".$value2A;
				/*********************Auto Transaction no*********************/
		 $sql              = "INSERT INTO `tbl_ams_assets`(`asset_no`, `company_id`, `location_id`, `emp_id`, `asset_type_id`,`os_id`,`asset_name`,`ip_address`, `comments`, `purchase_date`,`warranty_end_date`, `support_end_date`, `entered_by`) VALUES ('$ordervalue','$company_id','$location_id','$emp_id','$asset_type_id','$os_id','$asset_name','$ip_address','$comments',$purchase_date,$warranty_end_date,$support_end_date,'$userno')"; 
		  $result           = mysqli_query($conn,$sql);
		  $last_id          = mysqli_insert_id($conn);
		  if($result){
		   $sqlQry           = mysqli_query($conn,"INSERT INTO `tbl_ams_asset_warranty_history`(`asset_id`, `start_date`, `end_date`,`entered_by`) VALUES ('$last_id',$purchase_date,$warranty_end_date,'$userno')");
		 /************ITEMS DETAILS**************/	  
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <?php  
				  $d1 = mysqli_query($conn,"SELECT `company_name`, `company_id` FROM `tbl_emp_company_master` WHERE `is_active`=1 order by `company_name`");
                ...
                <div class="form-group">
                  ...
                  <div class="input-group date">
                    ...
                    <input type="text" class="validate[required] form-control" id="datepicker"  data-inputmask="'alias': 'dd-mm-yyyy'" data-mask="" placeholder="dd-mm-yyyy format" value="<?=date('d-m-Y')?>" name="purchase_date"> 
                  </div>
                </div>
                  ...
                  <div class="input-group date">
                    <div class="input-group-addon"> <i class="fa fa-calendar"></i> </div>
                    <input type="text" class="validate[required] form-control datepicker1" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask="" placeholder="dd-mm-yyyy format" value="" name="support_end_date"> 
                  </div>
                </div>
              ...
              <div class="col-md-6">
                ...
                <div class="form-group">
                  ...
                  <div class="input-group date">
                    <div class="input-group-addon"> <i class="fa fa-calendar"></i> </div>
                    <input type="text" class="validate[required] form-control datepicker1" data-inputmask="'alias': 'dd-mm-yyyy'" data-mask="" placeholder="dd-mm-yyyy format" value="" name="warranty_end_date"> 
                  </div>
                </div>
                      <div class="form-group">
