...
if ($refreshflag==3){
	...
	$company_address       = trim($_REQUEST['company_address']);
			...
			$thumbk                     = "ENG_".substr($document_signature, 0, $extension_pos) .'_'.time(). substr($document_signature, $extension_pos);
			 if ($_FILES["company_logo"]["error"] > 0 ){}      
				   ...
				   move_uploaded_file($_FILES["company_logo"]["tmp_name"],"../uploads/company_logo/".$thumbk); 
				    ...
				    move_uploaded_file($_FILES["company_logo"]["tmp_name"],"../uploads/inv_company_logo/".$thumbk); 
			   }     
			 }
		  $sql              = "INSERT INTO `tbl_inv_company_master`(`company_name`, `company_code`, `company_tr_no`, `company_logo`, `company_address`, `company_fax`, `company_phone`,`company_email`, `entered_by`) VALUES ('$company_name','$company_code','$company_tr_no','$thumbk','$company_address','$company_fax','$company_phone','$company_email','$userno')";
            ...
            <div class="col-md-12">
              <div class="table-responsive">
                <table id="example2" class="table table-bordered table-hover table-striped" role="grid" width="100%;">
                  ...
                  <tbody>
                    <?php 
				                           $sqlQ  = mysqli_query($conn,"SELECT * FROM `tbl_inv_company_master` ORDER BY `entered_date` DESC");
										   $itr = 0;
                                            while($build = mysqli_fetch_array($sqlQ)){
											$company_logo           = $build['company_logo'];
											$u_pathnF = '../uploads/inv_company_logo/'; 
