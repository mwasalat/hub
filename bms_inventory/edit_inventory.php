...
if ($refreshflag==3){
	...
	$inventory_master_id_now = $_REQUEST['inventory_master_id_now'];
	...
	$error_cnt=0;
	$IsPageValid = true;
	if(empty($inventory_master_id_now)){
	...
	if($IsPageValid==true){
		  $sql       = "UPDATE `tbl_bms_inventory_main_master` SET `inventory`='$inventory',`updated_by`='$userno',`updated_date`=NOW() WHERE `inventory_master_id`='$inventory_master_id_now'";
		  ...
		  /***Daywise price update******/
					 ...
					 while (strtotime($start_day) <= strtotime($end_day)) {
							...
							//$sqlA = mysqli_query($conn,"INSERT INTO `tbl_bms_inventory_master` (`inventory_master_id`, `broker_id`,`vehicle_id`, `location_id`, `inventory_date`, `inventory`,`is_active`,`entered_by`) VALUES ('".$last_id."','".$broker."','".$vehicle."','".$location."','".$start_day."','".$inventory."',1,'".$userno."')");
							$sqlA = mysqli_query($conn,"UPDATE `tbl_bms_inventory_master` SET `inventory`='".$inventory."',`updated_by`='$userno',`updated_date`=NOW() WHERE `inventory_master_id`='$inventory_master_id_now' AND `inventory_date`='$start_day'");
							if(empty($sqlA)) {
		  ...
		  /*******ERROR CHECK****************/
	                if ($error_cnt==0) {
						$ip_address       = getIP();	   
		                $log_query        = mysqli_query($conn,"INSERT INTO `tbl_bms_logs`(`log_remarks`,`log_module`,`ip_address`,`entered_by`)VALUES('BMS inventory master updation for Master ID:$inventory_master_id_now','16','$ip_address','$userno')");
                        $alert_label = "alert-success"; 
                        $msg = "Inventory updated successfully!";
                    } else {
                        $alert_label = "alert-danger"; 
                        $msg = "Inventory not updated completely!";
                   }
	   /**********************************/
...
$inventory_master_id = killstring($_REQUEST['flag']);
$qry= mysqli_query($conn,"SELECT tb1.*,tb2.`location_name`,tb3.`vehicle_name`,tb4.`broker_name`FROM `tbl_bms_inventory_main_master` tb1 LEFT JOIN `tbl_bms_location_master` tb2 ON tb1.`location_id`=tb2.`location_id` LEFT JOIN `tbl_bms_vehicle_master` tb3 ON tb1.`vehicle_id`=tb3.`vehicle_id` LEFT JOIN `tbl_bms_broker_master` tb4 ON tb1.`broker_id`=tb4.`broker_id` WHERE tb1.`inventory_master_id`='$inventory_master_id'");
while($rowA = mysqli_fetch_array($qry)){
	$inventory_master_id_now = $rowA['inventory_master_id'];
    ...
    <form role="form" name="company-form" id="company-form" method="post" action="#" enctype="multipart/form-data">
      ...
      <div class="box box-default">
        ...
        <div class="box-body">
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="inventory" class="validate[required,custom[number]] form-control" value="<?=$inventory?>"/>
              </div>
        ...
        <div class="box-footer">
          <a href="list_inventory.php?gtoken=<?=$token?>" class="btn btn-warning pull-left" title="Back">Back</a>
          ...
          <input type="hidden" name='inventory_master_id_now' value= '<?=$inventory_master_id_now?>' >
