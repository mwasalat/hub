...
<div class="content-wrapper">
  ...
  <section class="content-header">
   <a href="new_premium.php?gtoken=<?=$token?>" class="btn btn-warning" style="float:right"><i class="fa fa-plus"></i> New Premium - Individual</a>
   <h1>Premium List - Individual</h1>
                  ...
                  <thead>
                    <tr>
                     ...
                     <!-- <th >Geographical Coverage</th>
                      <th >Inpatient (Hospitalization)</th>
                      <th >Out Patient</th>
                      <th >Physiotherapy</th>
                       <th >Emergency Evacuation</th>
                        <th >Chronic Conditions</th>
                         <th >Pre-existing Cover</th>
                          <th >Routine Maternity</th>
                           <th >Maternity Waiting Period</th>
                            <th >Dental</th>
                             <th >Dental Waiting Period</th>
                              <th >Optical Benefits</th>
                               <th >Wellness</th>
                               <th >Semi Annual Surcharge</th>
                               <th >Quarterly Surcharge</th>
                               <th >Monthly Surcharge</th>-->
                      <th >Status</th>
                    </tr>
                  </thead>
                  <tbody>
                   <?php 
											$Query = "SELECT tb1.*,tb2.`company_name` FROM `tbl_cmt_premium_master` tb1 LEFT JOIN `tbl_cmt_ins_company_master` tb2 ON tb1.`company_id`=tb2.`company_id`  ORDER BY tb1.`premium_id` DESC";	
                    ...
                    <tr>
                      ...
                      <td><?=$build['premium_name']?></td>
                      <td><?=$build['annual_limit']?></td>
                      <?php /*?><td><?=$build['geographical_coverage']?></td>
                      <td><?=$build['inpatient']?></td>
                      <td><?=$build['out_patient']?></td>
                      <td><?=$build['physiotherapy']?></td>
                      <td><?=$build['emergency_evacuation']?></td>
                      <td><?=$build['chronic_conditions']?></td>
                      <td><?=$build['pre_existing_cover']?></td>
                      <td><?=$build['routine_maternity']?></td>
                      <td><?=$build['maternity_waiting_period']?></td>
                      <td><?=$build['dental']?></td>
                      <td><?=$build['dental_waiting_period']?></td>
                      <td><?=$build['optical_benefits']?></td>
                      <td><?=$build['wellness']?></td>
                      <td><?=$build['semi_annual_surcharge']?></td>  
                      <td><?=$build['quarterly_surcharge']?></td>  
                      <td><?=$build['monthly_surcharge']?></td>  <?php */?>
