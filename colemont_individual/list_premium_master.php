  ...
  <section class="content">
            ...
            <div class="col-md-12">
              <div class="table-responsive">
                <table id="example1" class="table table-bordered table-hover table-striped stripe row-border order-column" role="grid" style="font-size:12px;">
                  <tbody>
                    <?php
                    $Query = "SELECT tb1.*,tb2.`company_name`,COUNT(tb1.`premium_id`) AS `total_count` FROM `tbl_cmt_premium_master` tb1 LEFT JOIN `tbl_cmt_ins_company_master` tb2 ON tb1.`company_id`=tb2.`company_id` GROUP BY tb1.`company_id` ORDER BY tb1.`premium_id` DESC";
                        ...
                        <td>
                          <div class="col-lg-3 col-xs-6">
                            <div class="small-box bg-<?= $random_keys ?>">
                              ...
                              <a href="list_premium.php?flag1=<?= $build['company_id'] ?>&flag2=<?= $build['company_name'] ?>&gtoken=<?= $token ?>" title="View" class="small-box-footer"> More info <i class="fa fa-arrow-circle-right"></i></a>
                            </div>
                          </div>
                        </td>
                      </tr>
    ...
    <div class="row">
        ...
        while ($build = mysqli_fetch_array($sqlQ)) {
            $class      = ($build['is_active'] == "1") ? 'bg-success' : 'bg-danger';
            $label      = ($build['is_active'] == "1") ? 'label-success' : 'label-danger';
            $type       = ($build['is_active'] == "1") ? 'Active' : 'Inactive';
            $a           = array("red", "green", "blue", "yellow", "aqua");
            $random_keys =  $a[array_rand($a, 1)];
        ?>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('.datepickerA').inputmask('dd-mm-yyyy', {
