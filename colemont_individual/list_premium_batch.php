...
$msg = "";
$company_id   = killstring($_REQUEST['flag1']);
$company_name = killstring($_REQUEST['flag2']);
?>
<style>
 table#example2 {
  margin: 0 auto;
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>Premium List Batch - <?=$company_name?> </h1>
            ...
            <div class="col-md-12">
              <div class="table-responsive">
                <table id="example2"  class="table table-bordered table-hover table-striped stripe row-border order-column" role="grid" style="font-size:12px;">
                  ...
                  <tbody>
                   <?php 
											$Query = "SELECT tb1.*,tb2.`company_name`,tb3.`premium_name` FROM `tbl_cmt_ins_price_batch_master` tb1 LEFT JOIN `tbl_cmt_ins_company_master` tb2 ON tb1.`company_id`=tb2.`company_id`  INNER JOIN `tbl_cmt_premium_master`tb3 ON tb1.`premium_id`=tb3.`premium_id` WHERE tb1.`company_id`='$company_id' ORDER BY tb1.`price_batch_id` DESC";	
                    ...
                    <tr>
                      ...
                      <a href="list_premium_inner.php?flag=<?=$build['price_batch_id']?>&gtoken=<?=$token?>" class="btn btn-default" title="View"/><i class="fa fa-eye"></i></a>
                      <a href="edit_premium_batch.php?flag=<?=$build['price_batch_id']?>&gtoken=<?=$token?>" class="btn btn-default" title="Edit"/><i class="fa fa-edit"></i></a>
					  <?php }?>
                      </td>
                    </tr>
                    <?php 
