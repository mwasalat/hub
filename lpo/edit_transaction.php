...
if ($refreshflag==3){
	...
	$transaction_id_now          = killstring($_REQUEST['transaction_id_now']);
	$status_now                  = killstring($_REQUEST['status']);
	$cancel_reason               = killstring($_REQUEST['cancel_reason']);
	$IsPageValid = true;	
	if(empty($status_now)){
	...
	if($IsPageValid==true){//valid - true
		  $sql              = "UPDATE `tbl_transactions` SET `status`='$status_now', `cancel_reason`='$cancel_reason',`updated_by`='$userno',`updated_date`=NOW() WHERE `transaction_id`='$transaction_id_now'"; 
      ...
      <?php }?>
        ...
        <div class="box box-default" data-select2-id="16">
          <div class="box-header with-border">
            <h3 class="box-title">Transaction / <span style="color:red; font-weight:bold;"><?=$transaction_no?></span></h3>
          ...
          <div class="box-body">
            <div class="row">
              <div class="col-md-12">
                <div class="table-responsive">
                <hr/>
                <table class="table table-bordered"  width="100%" height="auto" border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;border:solid 1px #DDD"><tbody><tr><th colspan="5">LPO Items</th></tr>
                                           <?php
            ...
            <div class="row">
              ...
              <div class="form-group">
                  <label class="required">Status:</label>
                   <select name="status" class="validate[required] form-control" onchange="change_status(this.value)" <?php if($status==2) {?> disabled="disabled"<?php }?>/>
                   <option value="1" <?php if($status==1) {echo "selected";}?>>Issued</option>
                   <option value="2" <?php if($status==2) {echo "selected";}?>>Cancel</option>
                   </select>
                </div>
                <div class="form-group" id="cancel_reason_div" style="display:none;">
                  <label class="required">Reason:</label>
                   <textarea rows="4" name="cancel_reason" placeholder="Reason" class="validate[required] form-control"></textarea>
                </div>
              </div>
            </div>
            <!-- /.row --> 
          ...
          <input type="hidden" name='transaction_id_now' value= '<?=$transaction_id?>' >
          <button type="submit" class="btn btn-primary pull-right" name="submit" id="cancel_button" style="display:none;">Cancel</button>
          </div>
        </div>
        <!-- /.box -->
        <!-- /.row -->
