...
if ($refreshflag==3){
	...
	$trn_type       = killstring($_REQUEST['trn_type']);
	$place_of_delivery       = killstring($_REQUEST['place_of_delivery']);
	$delivery_schedule       = killstring($_REQUEST['delivery_schedule']);
	$place_of_registration   = killstring($_REQUEST['place_of_registration']);
	$terms_of_payment        = killstring($_REQUEST['terms_of_payment']);
	...
	if($IsPageValid==true){//valid - true
	              /******************ompany Code Find**********/
				  $qry1         = mysqli_query($conn,"SELECT `company_code` FROM `tbl_company_master` WHERE `company_id`='$company'");
				  $row          = mysqli_fetch_row($qry1);
				  $company_code = $row['0'];
				 /******************ompany Code Find**********/
				  
	             /******************Auto Transaction no***********************/
			        $QueryCounter  =  mysqli_query($conn,"SELECT `auto_id`+1 as `id` from `tbl_transaction_no`");
					$rowQryA       = mysqli_fetch_row($QueryCounter);
					$value2A       = $rowQryA[0];
					$QueryCounterA =  mysqli_query($conn,"UPDATE `tbl_transaction_no` set `auto_id`='$value2A'");
					$ordervalue    = $company_code."-".$trn_type."-".$value2A;
				/*********************Auto Transaction no*********************/
		 $sql              = "INSERT INTO `tbl_transactions`(`transaction_no`, `company_id`, `supplier_id`, `date_of_entry`, `total_value`,`vat_value`,`trn_type`,`place_of_delivery`, `delivery_schedule`, `place_of_registration`,`terms_of_payment`, `special_terms`, `comments`, `entered_by`) VALUES ('$ordervalue','$company','$supplier',$entry_date,'$products_total_value','$products_vat_value','$trn_type','$place_of_delivery','$delivery_schedule','$place_of_registration','$terms_of_payment','$special_terms','$comments','$userno')"; 
		 ...
		 /************ITEMS DETAILS**************/
		  ...
		  for($i=0; $i < $cntC; $i++){
		   ...
		   $products_description   = killstring(utf8_decode($products_description));
		   ...
		   $products_unit_price    = !empty($products_unit_price) ? $products_unit_price : 0;
			...
			$sqlQry           = mysqli_query($conn,"INSERT INTO `tbl_transactions_items`(`transaction_id`, `item_description`, `item_quantity`, `item_unit_value`, `item_total_value`,`entered_by`) VALUES ('$last_id','$products_description','$products_quantity','$products_unit_price','$products_total_price','$userno')");
			}
		 }
		 /************ITEMS DETAILS**************/	  
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <?php  
				  $d1 = mysqli_query($conn,"SELECT `company_name`, `company_id` FROM `tbl_company_master` WHERE `is_active`=1 order by `company_name`");
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <?php  
				  $d1 = mysqli_query($conn,"SELECT `supplier_name`, `supplier_id` FROM `tbl_supplier_master` WHERE `is_active`=1 order by `supplier_name`");
                  while($b1 = mysqli_fetch_array($d1)){
				  ?>
                ...
                <div class="form-group">
                  ...
                  <select class="validate[required] form-control" name="trn_type" id="trn_type">
                  <option selected="selected" value="">---Select a Type---</option>
                 <option value="A">Airfare</option>
            ...
            <div class="row">
              <div class="col-md-12"><hr/>
                <div class="input-group"> <span class="input-group-btn">
                  <button class="btn btn-default" type="button" name="feature_btn" onClick="add_items();">+ Add LPO Item</button>
                  </span> </div>
              </div>
              <div class="col-md-12">
                <div class="table-responsive">
                  <table class="table table-bordered table-hover table-sriped" role="grid">
                    ...
                    <tfoot>
                      <tr>
                        <td><input type="hidden" name="products_sub_total_value_hidden" value="0" id="products_sub_total_value_hidden"/><b>Sub Total Value</b></td>
                        <td></td>
                        <td></td>
                        <td><span id="products_sub_total_value" style="font-weight:bold; color:#990000;"></span></td>
                        <td></td>
                      </tr>
                      <tr><td colspan="5"><label>Apply 5% VAT:&nbsp;&nbsp;<input type="checkbox" id="vat_flag" value="1" onchange="TotalFill();"></label></td></tr>
                      <tr><td colspan="5"> <label>VAT:&nbsp;&nbsp;<input type="hidden" name="products_vat_hidden" value="0" id="products_vat_hidden"/><span id="products_vat_value" style="font-weight:bold; color:#990000;">0</span></label></td></tr>
                      <tr><td colspan="5">  <label>Total Value:&nbsp;&nbsp;<input type="hidden" name="products_total_value_hidden" value="0" id="products_total_value_hidden"/><span id="products_total_value" style="font-weight:bold; color:#990000;">0</span></label></td></tr>
          ...
          <!-- /.box-body -->
           ...
           <div class="box-body">
            <div class="row">
              <div class="col-md-6">
              <div class="form-group">
                  <label class="required">Place Of Delivery:</label>
                    <input type="text" class="validate[required] form-control" placeholder="Place Of Delivery" value="" name="place_of_delivery">
                </div>
                <div class="form-group">
                  <label class="required">Delivery Schedule:</label>
                 <input type="text" class="validate[required] form-control" id="delivery_schedule" placeholder="Delivery Schedule" value="" name="delivery_schedule">
                </div>
                <div class="form-group">
                  <label class="">Special Terms & Conditions:</label>
                  <textarea rows="4" name="special_terms" placeholder="Special Terms & Conditions" class="form-control"></textarea>
                </div>
              </div>
              <div class="col-md-6">
              <div class="form-group">
                  <label class="">Place Of Registration:</label>
                    <input type="text" class="form-control" placeholder="Place Of Registration" value="" name="place_of_registration">
                </div>
                <div class="form-group">
                  <label class="">Terms Of Payment:</label>
                   <input type="text" class="form-control" placeholder="Terms Of Payment" value="" name="terms_of_payment">
                </div>
                <div class="form-group">
